...
//--- Trade Mode (Stacking / Scale-In)
enum ENUM_TRADE_MODE
{
   MODE_SINGLE=0,        // Single Trade Only (Classic)
   MODE_SCALE_IN=1       // Structural Scale-In (Dynamic SL/TP)
};
...
//--- SMC Timeframes Custom Enum
enum ENUM_SMC_TIMEFRAME
{
   SMC_TF_CURRENT=0,                // Current
   SMC_TF_H1=PERIOD_H1,             // H1
   SMC_TF_H4=PERIOD_H4,             // H4
   SMC_TF_D1=PERIOD_D1,             // D1
   SMC_TF_W1=PERIOD_W1,             // 1W
   SMC_TF_MN1=PERIOD_MN1            // 1Month
};
...
input ENUM_TRADE_MODE InpTradeMode       = MODE_SINGLE;   // Trading Mode
...
input ENUM_SMC_TIMEFRAME InpStructureTimeframe = SMC_TF_CURRENT; // Structure TimeFrame
...
input color  InpBOSColor         = clrWhite; // Color Líneas BOS
...
int OnInit()
{
   ...
   ENUM_TIMEFRAMES zzTimeframe = (InpStructureTimeframe == SMC_TF_CURRENT) ? _Period : (ENUM_TIMEFRAMES)InpStructureTimeframe;
   if(InpZigZagType == ZZ_STANDARD)
      handleZigZag = iCustom(_Symbol, zzTimeframe, "Examples\\ZigZag", InpZZ_Depth, InpZZ_Deviation, InpZZ_Backstep);
   else // ZZ_ADAPTIVE_SWING
      handleZigZag = iCustom(_Symbol, zzTimeframe, "AdaptiveSwingDetector", InpAdaptiveATR, InpAdaptiveK, InpAdaptiveMinBars, true);
...
void OnTick()
{
   ...
   //=================================================================
   //                    FILTRO ANTI-HEDGING & MODE SINGLE
   //=================================================================
   if(InpBlockOppositeDirections)
   {
      if(HasOppositeDirectionTrade(1)) canBuy = false;
      if(HasOppositeDirectionTrade(-1)) canSell = false;
   }
   
   if(InpTradeMode == MODE_SINGLE)
   {
      if(HasSameDirectionTrade(1)) canBuy = false;
      if(HasSameDirectionTrade(-1)) canSell = false;
   }
...
bool GetZigZagSwing(int bufferIndex, double &price, datetime &time)
{
   ...
   for(int i = copied - 1; i >= 0; i--)
   {
      if(buffer[i] != 0 && buffer[i] != EMPTY_VALUE)
      {
         ...
         ENUM_TIMEFRAMES zzTimeframe = (InpStructureTimeframe == SMC_TF_CURRENT) ? _Period : (ENUM_TIMEFRAMES)InpStructureTimeframe;
         time = iTime(_Symbol, zzTimeframe, shift);
         return true;
      }
   }
   return false;
...
double GetLastZigZagLow(datetime &time)
{
   double price;
...
{
   datetime t;
   return GetLastZigZagLow(t);
}

double GetLastZigZagHigh(datetime &time)
{
   double price;
...
//--- Obtener dirección de estructura basada en ZigZag Adaptive (Con filtro BOS - SMC)
// Retorna: 1 (Alcista), -1 (Bajista), 0 (Indefinido)
int GetStructureDirection()
{
   datetime lastHighTime=0, lastLowTime=0;
   double lastHigh = GetLastZigZagHigh(lastHighTime);
   double lastLow = GetLastZigZagLow(lastLowTime);
   
   // Necesitamos al menos un High y un Low para empezar
   if(lastHigh == 0 || lastLow == 0) return 0;
   
   datetime prevHighTime=0, prevLowTime=0;
   double prevHigh = GetPreviousZigZagHigh(lastHigh, prevHighTime);
   double prevLow = GetPreviousZigZagLow(lastLow, prevLowTime);
   
   // 1. Condición de Soportes/Resistencias (Higher Lows / Lower Highs)
   bool higherLow = (prevLow > 0 && lastLow > prevLow);
   bool lowerHigh = (prevHigh > 0 && lastHigh < prevHigh);
   
   // 2. Condición de Break of Structure (BOS)
   bool bosUp = (prevHigh > 0 && lastHigh > prevHigh);   // El High reciente rompió el previo
   bool bosDown = (prevLow > 0 && lastLow < prevLow);    // El Low reciente rompió el previo
   
   // Estructura Alcista Confirmada (BOS Alcista + Higher Low)
   if(bosUp && higherLow) 
   {
      DrawBOSLine(prevHighTime, prevHigh, lastHighTime, "BOS_UP_", InpBOSColor);
      return 1;
   }
   
   // Estructura Bajista Confirmada (BOS Bajista + Lower High)
   if(bosDown && lowerHigh) 
   {
      DrawBOSLine(prevLowTime, prevLow, lastLowTime, "BOS_DN_", InpBOSColor);
      return -1;
   }
   
   // Si no hay BOS claro o estamos en compresión/expansión simétrica
   return 0; 
}
//=======================================================================
...
string GetStructureStr()
{
   ...
   if(dir == 1) return "BULLISH (BOS + HL)";
   if(dir == -1) return "BEARISH (BOS + LH)";
   return "NEUTRAL / NO CONFIRMATION";
}
...
double GetPreviousZigZagHigh(double currentHigh, datetime &timeOut)
{
   double buffer[];
   int barsToCopy = 1000;
   ...
   for(int i = barsToCopy - 1; i >= 0; i--)
   {
      if(buffer[i] != 0 && buffer[i] != EMPTY_VALUE)
      {
         ...
         int shift = barsToCopy - 1 - i;
         ENUM_TIMEFRAMES zzTimeframe = (InpStructureTimeframe == SMC_TF_CURRENT) ? _Period : (ENUM_TIMEFRAMES)InpStructureTimeframe;
         timeOut = iTime(_Symbol, zzTimeframe, shift);
         return buffer[i];
      }
   }
   return 0;
...
{
   datetime t;
   return GetPreviousZigZagHigh(currentHigh, t);
}

//--- Obtener el Low ANTERIOR al último (para comparar HL/LL)
double GetPreviousZigZagLow(double currentLow, datetime &timeOut)
{
   double buffer[];
   int barsToCopy = 1000;
...
{
   datetime t;
   return GetPreviousZigZagLow(currentLow, t);
}

void DrawBOSLine(datetime time1, double price, datetime time2, string namePrefix, color clr)
{
   string objName = namePrefix + IntegerToString(time1);
   ...
   if(ObjectFind(0, objName) < 0)
   {
      ...
      ObjectCreate(0, textName, OBJ_TEXT, 0, time2, price);
      
      ENUM_TIMEFRAMES zzTimeframe = (InpStructureTimeframe == SMC_TF_CURRENT) ? _Period : (ENUM_TIMEFRAMES)InpStructureTimeframe;
      string tfStr = EnumToString(zzTimeframe);
      StringReplace(tfStr, "PERIOD_", "");
      ObjectSetString(0, textName, OBJPROP_TEXT, "  BOS " + tfStr);
...
//--- Verificar si existe trade en LA MISMA dirección
bool HasSameDirectionTrade(int direction) // 1=Buy, -1=Sell
{
...
void ManagePyramiding()
{
   ...
   if(InpTradeMode != MODE_SCALE_IN) return; // Solo funciona en modo Scale-In
         ...
         if(positionInfo.Symbol() == _Symbol && positionInfo.Magic() == InpMagicNumber)
         {
            if(positionInfo.PositionType() == POSITION_TYPE_BUY)
            {
               if(positionInfo.Time() > newestBuyTime)
               {
                  ...
                  newestBuySL = positionInfo.PriceOpen(); // <-- SL de trades viejos movido al precio de entrada del nuevo
            ...
            else if(positionInfo.PositionType() == POSITION_TYPE_SELL)
            {
               if(positionInfo.Time() > newestSellTime)
               {
                  ...
                  newestSellSL = positionInfo.PriceOpen(); // <-- SL de trades viejos movido al precio de entrada del nuevo
